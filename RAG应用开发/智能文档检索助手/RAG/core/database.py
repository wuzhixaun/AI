...
class ChildChunk(Base):
    ...
    document_id = Column(Integer,ForeignKey("documents.id"),nullable=False) # 文档id,不为空
    parent_chunk_id = Column(Integer,ForeignKey("parent_chunks.id"),nullable=False) # 父文档id,不为空
...
class ChatHistory(Base):
    ...
    document_ids = Column(String(500)) # 相关文档id,不为空
    used_chunks = Column(Text) # 使用过的子文档id,不为空
    created_at = Column(DateTime,default=datetime.now) # 创建时间,不为空
...
class DatabaseManager:
    ...
    def save_chat_history(self,session_id,user_message,assistant_message,document_ids=None,used_chunks=None):
        ...
        session = self.get_session()
        try:
            chat_history = ChatHistory(session_id=session_id,user_message=user_message,assistant_message=assistant_message,document_ids=document_ids,used_chunks=used_chunks)
